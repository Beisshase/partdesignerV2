...
class Editor {
	...
	constructor() {
		...
		document.getElementById("save-part").addEventListener("click", (event: MouseEvent) => this.savePart());
		document.getElementById("load-part").addEventListener("click", (event: MouseEvent) => this.loadPart());
		document.getElementById("load-part-input").addEventListener("change", (event: Event) => this.onLoadPartFile(event));
	...
	private getRevision(): string {
		const rev = (document.getElementById('partRevision') as HTMLInputElement).value.trim();
		return rev.length > 0 ? rev : '1.0';
	}

	private savePart() {
		const data = { name: this.getName(), revision: this.getRevision(), part: this.part.toString() };
		const blob = new Blob([JSON.stringify(data)], { type: 'application/json' });
		const url = URL.createObjectURL(blob);
		const a = document.createElement('a');
		a.href = url;
		a.download = this.getName() + '_v' + this.getRevision() + '.partds';
		a.click();
		URL.revokeObjectURL(url);
	}

	private loadPart() {
		(document.getElementById('load-part-input') as HTMLInputElement).click();
	}

	private onLoadPartFile(event: Event) {
		...
		reader.onload = (e) => {
			try {
				...
				this.part = Part.fromString(data.part);
				if (data.name) {
					this.setName(data.name);
				}
				if (data.revision) {
					(document.getElementById('partRevision') as HTMLInputElement).value = data.revision;
				}
				this.updateMesh(true);
			} catch {
				alert('Could not load part file.');
			}
			input.value = '';
		};
